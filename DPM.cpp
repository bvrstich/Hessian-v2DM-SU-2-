...
void DPM::init(){
   ...
   for(int S_ab = 0;S_ab < 2;++S_ab){//loop and allocate
      ...
      s2dp[0][S_ab] = new int ** [M];

      for(int a = 0;a < M;++a){

         s2dp[0][S_ab][a] = new int * [M];

         for(int b = 0;b < M;++b)
            s2dp[0][S_ab][a][b] = new int [M];
   ...
   for(int a = 0;a < M;++a){//loop and allocate
      ...
      s2dp[1][0][a] = new int * [M];

      for(int b = 0;b < M;++b)
         s2dp[1][0][a][b] = new int [M];
 ...
 */
DPM::DPM() : BlockMatrix(2) {

   //set the dimension and the degeneracies of the blocks
   this->setMatrixDim(0,dp2s[0].size(),2);
   this->setMatrixDim(1,dp2s[1].size(),4);
